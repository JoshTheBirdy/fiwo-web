...
(function () {
    ...
    // The course is 258 KB of script that only this page reads. It used to be a
    // plain <script> in index.html, so every visit that only wanted to look up
    // a word downloaded and ran it (audit 2026-09-23). It is fetched the first
    // time the workbook opens instead; the URL, ?v= and all, sits on the
    // section's data-src beside the other cache-busters. sw.js still precaches
    // it, so the lesson you were on opens offline.
    let courseLoading = null;
    let pendingDetail = null;

    function loadCourse() {
        if (window.FIWO_WORKBOOK) return Promise.resolve(window.FIWO_WORKBOOK);
        if (!courseLoading) {
            courseLoading = new Promise((resolve) => {
                const s = document.createElement('script');
                s.src = $('#workbook')?.dataset.src || 'workbook.js';
                s.onload = () => resolve(window.FIWO_WORKBOOK || null);
                // Forget the failure, so coming back to the page tries again.
                s.onerror = () => { courseLoading = null; s.remove(); resolve(null); };
    ...
    function open(detail) {
        const main = $('#wb-main');
        if (!main) return;
        if (!window.FIWO_WORKBOOK) {
            // The latest request wins: #workbook/L3 then L4 before the course
            // arrives should land on L4, once.
            pendingDetail = detail;
            main.innerHTML = '<p class="fc-note">Loading the course…</p>';
            loadCourse().then((course) => {
                if (!course) {
                    main.innerHTML = '<p class="fc-note">The course could not load. '
                        + 'Check your connection, then open the workbook again.</p>';
                    return;
                }
                // The reader may have moved on while it loaded.
                if ($('#workbook')?.classList.contains('active')) open(pendingDetail);
            });
            return;
        }
